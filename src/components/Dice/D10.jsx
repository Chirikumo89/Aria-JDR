...
export default function D10({ result = 1, rolling = false, position = [0, 0, 0] }) {
  ...
  const textures = useMemo(() => {
    ...
    return {
      ...
      topRight: createFaceTexture(7), // Face haut-droite
      topLeft: createFaceTexture(8),  // Face haut-gauche
      bottomRight: createFaceTexture(9), // Face bas-droite
      bottomLeft: createFaceTexture(0)   // Face bas-gauche (0 pour 10)
    };
  }, []);
  ...
  useFrame((state, delta) => {
    ...
    if (rollingRef.current) {
      ...
      // Ground collision
      if (positionRef.current.y <= -1.8) {
        positionRef.current.y = -1.8;
        velocityRef.current.y *= -bounce;
        velocityRef.current.x *= friction;
        velocityRef.current.z *= friction;
        
        // Add random movement after bounce
        if (Math.abs(velocityRef.current.y) < 0.08) {
          velocityRef.current.x += (Math.random() - 0.5) * 0.08;
          velocityRef.current.z += (Math.random() - 0.5) * 0.08;
        }
      }
  ...
  return (
    <group ref={meshRef}>
      <mesh scale={[1.1, 1.1, 1.1]} castShadow receiveShadow>
      ...
      {/* Textures sur chaque face de l'octaèdre */}
      {/* Face du haut (1) */}
      ...
      {/* Face arrière (4) */}
      <mesh position={[0, 0, -0.8]} rotation={[0, Math.PI, 0]}>
        <planeGeometry args={[1, 1]} />
      ...
      {/* Face haut-droite (7) */}
      <mesh position={[0.5, 0.5, 0.5]} rotation={[-Math.PI / 4, Math.PI / 4, 0]}>
        <planeGeometry args={[1, 1]} />
        <meshBasicMaterial map={textures.topRight} />
      </mesh>
      
      {/* Face haut-gauche (8) */}
      <mesh position={[-0.5, 0.5, -0.5]} rotation={[-Math.PI / 4, -Math.PI / 4, 0]}>
        <planeGeometry args={[1, 1]} />
        <meshBasicMaterial map={textures.topLeft} />
      </mesh>
      
      {/* Face bas-droite (9) */}
      <mesh position={[0.5, -0.5, -0.5]} rotation={[Math.PI / 4, Math.PI / 4, 0]}>
        <planeGeometry args={[1, 1]} />
        <meshBasicMaterial map={textures.bottomRight} />
      </mesh>
      
      {/* Face bas-gauche (0) */}
      <mesh position={[-0.5, -0.5, 0.5]} rotation={[Math.PI / 4, -Math.PI / 4, 0]}>
        <planeGeometry args={[1, 1]} />
        <meshBasicMaterial map={textures.bottomLeft} />
      </mesh>
    </group>
  );
}
